    ...
    <div class="register-card">
      ...
      <form @submit.prevent="handleRegister">
        <div class="form-group">
          ...
          <input type="text" v-model="company_name" placeholder="Enter company name" required />
        </div>

        <div class="form-group">
          <label>Email</label>
          <input type="email" v-model="email" placeholder="Enter company email" required />
        </div>
...
export default {
  ...
  methods: {
    async handleRegister() {
      ...
      try {
        var response = await fetch('http://localhost:5000/api/register/company', {
          method: 'POST',
          headers: {
